...
impl Parser {
    ...
    /// Start recording structural [`TermEvent`](crate::TermEvent)s. Off by
    /// default so parsers that never drain don't accumulate them.
    pub fn enable_events(&mut self) {
        self.screen.enable_events();
    }

    /// Take the events recorded since the last drain, in occurrence order.
    pub fn drain_events(&mut self) -> Vec<crate::TermEvent> {
        self.screen.drain_events()
    }
}
