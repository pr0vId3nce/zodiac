...
impl Screen {
    ...
    /// A ScrollUp/ScrollDown event for the current grid's scroll region.
    fn ev_scroll(&mut self, up: bool, top: u16, bottom: u16, n: u16) {
        if n == 0 || !self.events_on {
            return;
        }
        let event = if up {
            crate::event::TermEvent::ScrollUp { top, bottom, n }
        } else {
            crate::event::TermEvent::ScrollDown { top, bottom, n }
        };
        self.events.push(event);
    }
    ...
    pub(crate) fn drain_events(&mut self) -> Vec<crate::event::TermEvent> {
        std::mem::take(&mut self.events)
    }

    pub(crate) fn set_size(&mut self, rows: u16, cols: u16) {
        let size = self.grid.size();
        if (size.rows, size.cols) != (rows, cols) {
            self.ev(crate::event::TermEvent::Resize { rows, cols });
        }
    ...
    fn enter_alternate_grid(&mut self) {
        ...
        if !self.mode(MODE_ALTERNATE_SCREEN) {
            self.ev(crate::event::TermEvent::AltEnter);
        }
    ...
    fn exit_alternate_grid(&mut self) {
        if self.mode(MODE_ALTERNATE_SCREEN) {
            self.ev(crate::event::TermEvent::AltExit);
        }
...
impl Screen {
    ...
    fn ris(&mut self) {
        ...
        *self = Self::new(self.grid.size(), self.grid.scrollback_len());
        ...
        self.ev(crate::event::TermEvent::Reset);
    }
    ...
    fn ed(&mut self, mode: u16) {
        ...
        match mode {
            ...
            2 => {
                ...
                self.ev(crate::event::TermEvent::EraseScreen);
            }
            3 => self.ev(crate::event::TermEvent::EraseScreen),
            n => {
